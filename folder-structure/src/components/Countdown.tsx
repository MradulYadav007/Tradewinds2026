...
export function Countdown() {
  ...
  return <section className="border-y border-line py-8"><div className="page-shell flex flex-wrap items-center justify-between gap-6"><p className="eyebrow">{time === null ? closedLabel : label}</p><div><p className="font-display text-5xl font-black tracking-wide tabular-nums md:text-6xl">{time === null ? '00 : 00 : 00 : 00' : time}</p><p className="mt-3 flex justify-between text-xs tracking-wider text-muted"><span>DAYS</span><span>HOURS</span><span>MINUTES</span><span>SECONDS</span></p></div></div></section>;
}
